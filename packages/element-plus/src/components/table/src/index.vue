<template>
              ...
              <div style="display: flex">
                ...
                <div>
                  ...
                  <div class="action-icon" v-else>
                    <div class="check" @click.stop="check(scope)"></div>
                    <div class="close" @click.stop="close(scope)"></div>
                  </div>
                </div>
              </div>
            </template>
  ...
  <div v-if="pagination && !isLoading" class="pagination" :style="{ justifyContent }">
    <el-pagination
      v-model:currentPage="nowCurrentPage"
...
import { ElTable, ElPagination, ElTableColumn, ElInput  } from 'element-plus';
defineOptions({ name: 'ea-table' })
...
let nowCurrentPage = ref<number>(props.currentPage)
// 拷贝一份表格的数据
